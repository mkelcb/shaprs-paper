...
/lustre/scratch115/realdata/mdt3/projects/ukbiobank/FullRelease/
...
ukbbQCkeeplist='/lustre/scratch123/hgi/mdt1/projects/crohns/mk23/0Thesis/0_common/data/ukbb_hq_keeplist.txt' # Headerless file, col1: TeamID (for imputed data) col2: PublicID (for the genotypedata): 
...
rm -rf $outPutLocation$'.out'
rm -rf $outPutLocation$'.err'
if [ "$popALDpanel" = "0" ]; then
echo "NO LD PANEL PROVIDED"
else
largePlinkMem=320000
...
bsub -q long -m "modern_hardware" -G team152 -n16 -J LDrefgen -R "span[hosts=1] select[mem>${largePlinkMem}] rusage[mem=${largePlinkMem}]" -M${largePlinkMem} -o $outPutLocation$'.out' -e $outPutLocation$'.err'  "/nfs/team152/mk23/software/R_X_farm/R-4.1.0/bin/Rscript $arguments"
