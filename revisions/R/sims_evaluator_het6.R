...
allResults <- allResults[, c(2, 1, 4, 5, 3)] # reorder it, make single to be first, meta next, then smtpred then shaprs
...
grid.text( expression(bold(r^2) )  , x= 0.72, y = 0.98, gp = gpar(fontsize = 16, fontfamily = "sans"))
...
grid.text( expression(bold(r^2) )  , x= 0.72, y = 0.97, gp = gpar(fontsize = 16, fontfamily = "sans"))
dev.off()
...
# shaPRS was best in 94% of the simulation scenarios"

sink(paste0(outputLoc,"_overall.txt"))
print(paste0("meta: ", median(allResults$meta)))
print(paste0("proxmial: ", median(allResults$`proximal only`)))
print(paste0("shaPRS: ", median(allResults$shaPRS)))
print(paste0("SMTPred: ", median(allResults$SMTPred)))
print(paste0("MTAG: ", median(allResults$MTAG) ))
