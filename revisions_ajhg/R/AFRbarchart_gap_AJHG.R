...
createPlot = function(prsResults, prsResults_sd_low, prsResults_sd_high, ylabel, filn="r2", showHeight =T, plotWidth=1280, xlimit = c(0,45), minY = 0, cexV = 1.5, showValues =F, maxY =-1) {
  ...
  if(maxY == -1) {maxY = max(prsResults) + max(prsResults) * 0.1} 
  ...
  legend("topright", bty="n", ncol = 1, cex =cexV, y.intersp=1.8,
   ...
   # CHANGE for AJHG: we now decided to use r2redux to estimate error bars, so we now show it even for r^2s too!
   # add error bars
...
prsResults_r2= read.csv("C:/Users/M Kel/GoogleDrive_Cam/0Publications/shaPRS/Figs/revisions/AFR_res.csv", header = T,row.names = 1)
...
prsResults_r2_low= read.csv("C:/Users/M Kel/GoogleDrive_Cam/0Publications/shaPRS/Figs/revisions_ajhg/AFR_res_LOW.csv", header = T,row.names = 1)
prsResults_r2_high= read.csv("C:/Users/M Kel/GoogleDrive_Cam/0Publications/shaPRS/Figs/revisions_ajhg/AFR_res_HIGH.csv", header = T,row.names = 1)
outputLoc="C:/Users/M Kel/GoogleDrive_Cam/0Publications/shaPRS/Figs/revisions_ajhg/AFR_res_gap"

# add dummy gaps to separate out PRS-CSx
prsResults_r2 = addGap(prsResults_r2)
prsResults_r2_low = addGap(prsResults_r2_low)
prsResults_r2_high = addGap(prsResults_r2_high)

maxY = max(prsResults_r2) + max(prsResults_r2) *0.3
createPlot(prsResults_r2, prsResults_r2_low, prsResults_r2_high, "r^2", filn="r2", T, cexV = 1.2, maxY = maxY) # 

######################

# EAS-EUR trans-ethnic results:
# parse results into 'copy pastable' from Elena's output
resultsLoc= read.table("C:/0Datasets/shaPRS/Elena/Elena_201023/r2_ci.txt", header = T)

outputLoc="C:/Users/M Kel/GoogleDrive_Cam/0Publications/shaPRS/Figs/revisions_ajhg/gap"

pheno="height"

# LDpred2
resultsLoc_pheno = cbind.data.frame(paste0(pheno,"_EUR_EUR_LDpred2_CSxsubset_E"), 0,0,0,0.0976, 0.0954,0.0998,pheno)

# PRS-CS
df = cbind.data.frame(paste0(pheno,"_EUR_EUR_PRSCS_E"), 0,0,0,0.116, 0.114,0.118,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)

# PRS-CSx
df = cbind.data.frame(paste0(pheno,".PRSCSx_E"), 0,0,0,0.123, 0.12, 0.127,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)

# PRS-CSx-stage1
df = cbind.data.frame(paste0(pheno,"_EUR_PRSCSx_E"), 0,0,0,0.121, 0.118, 0.123,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)

# shaPRS + LDpred2
df = cbind.data.frame(paste0(pheno,"_shaPRS_LDpred2_CSxsubset_E"), 0,0,0,0.122, 0.119, 0.124,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)

# shaPRS + PRS-CS
df = cbind.data.frame(paste0(pheno,"_shaPRS_EUR_PRSCS_E"), 0,0,0,0.122, 0.119, 0.124,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)
...
pheno="asthma"
#LDpred2
resultsLoc_pheno = cbind.data.frame(paste0(pheno,"_EUR_EUR_LDpred2_CSxsubset_E"), 0.595,0.5919,0.5989 ,0.0115, 0.0107, 0.0124,pheno)

# PRS-CS
df = cbind.data.frame(paste0(pheno,"_EUR_EUR_PRSCS_E"), 0.588,0.5849,0.5918,0.00986, 0.0114, 0.0132,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)

# PRS-CSx
df = cbind.data.frame(paste0(pheno,".PRSCSx_E"), 0.603,0.5981,0.6079,0.0134,0.0121,0.0147,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)

# PRS-CSx-stage1
df = cbind.data.frame(paste0(pheno,"_EUR_PRSCSx_E"), 0.595,0.5916,0.5985 ,0.0113, 0.0105, 0.0122,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)

# shaPRS+LDpred2
df = cbind.data.frame(paste0(pheno,"_shaPRS_LDpred2_CSxsubset_E"), 0.604,0.6004,0.6073,0.0136, 0.0128, 0.0146,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)

# shaPRS+PRS-CS
df = cbind.data.frame(paste0(pheno,"_shaPRS_EUR_PRSCS_E"), 0.599,0.5951,0.602,0.0123, 0.0114, 0.0132,pheno)
colnames(df) = colnames(resultsLoc_pheno)
resultsLoc_pheno = rbind(resultsLoc_pheno,df)
...
prsResults_r2 = NULL
prsResults_AUC = NULL
prsResults_r2_low = NULL
prsResults_r2_high = NULL
...
for (i in 1:length(traits)) {
  ...
  primary_ldpred2_r2_low = resultsLoc_pheno$r2_low[grep("_EUR_EUR_LDpred2_CSxsubset", resultsLoc_pheno$model)]
  primary_ldpred2_r2_high = resultsLoc_pheno$r2_high[grep("_EUR_EUR_LDpred2_CSxsubset", resultsLoc_pheno$model)]
  ...
  primary_prscs_r2_low = resultsLoc_pheno$r2_low[grep("_EUR_EUR_PRSCS_E", resultsLoc_pheno$model)]
  primary_prscs_r2_high = resultsLoc_pheno$r2_high[grep("_EUR_EUR_PRSCS_E", resultsLoc_pheno$model)]
  ...
  pooledData_prscsx_r2_low = resultsLoc_pheno$r2_low[grep(".PRSCSx_E", resultsLoc_pheno$model, fixed=T)]
  pooledData_prscsx_r2_high = resultsLoc_pheno$r2_high[grep(".PRSCSx_E", resultsLoc_pheno$model, fixed=T)]
  ...
  pooledData_prscsx_unweighted_r2_low = resultsLoc_pheno$r2_low[grep("_EUR_PRSCSx_E", resultsLoc_pheno$model, fixed=T)]
  pooledData_prscsx_unweighted_r2_high = resultsLoc_pheno$r2_high[grep("_EUR_PRSCSx_E", resultsLoc_pheno$model, fixed=T)]
  ...
  pooledData_shaprs_ldpred2_unweighted_r2_low = resultsLoc_pheno$r2_low[grep("_shaPRS_LDpred2_CSxsubset_E", resultsLoc_pheno$model, fixed=T)]
  pooledData_shaprs_ldpred2_unweighted_r2_high = resultsLoc_pheno$r2_high[grep("_shaPRS_LDpred2_CSxsubset_E", resultsLoc_pheno$model, fixed=T)]
  ...
  pooledData_shaprs_prcs_unweighted_r2_low = resultsLoc_pheno$r2_low[grep("_shaPRS_EUR_PRSCS_E", resultsLoc_pheno$model, fixed=T)]
  pooledData_shaprs_prcs_unweighted_r2_high = resultsLoc_pheno$r2_high[grep("_shaPRS_EUR_PRSCS_E", resultsLoc_pheno$model, fixed=T)]
  ...
  prsResults_r2_low = cbind(prsResults_r2_low,   c(primary_ldpred2_r2_low, primary_prscs_r2_low, pooledData_shaprs_ldpred2_unweighted_r2_low, pooledData_shaprs_prcs_unweighted_r2_low, pooledData_prscsx_r2_low, pooledData_prscsx_unweighted_r2_low ))
  prsResults_r2_high = cbind(prsResults_r2_high,   c(primary_ldpred2_r2_high, primary_prscs_r2_high, pooledData_shaprs_ldpred2_unweighted_r2_high, pooledData_shaprs_prcs_unweighted_r2_high, pooledData_prscsx_r2_high, pooledData_prscsx_unweighted_r2_high ))
...
rownames(prsResults_r2_low) = rownames_trait
rownames(prsResults_r2_high) = rownames_trait
rownames(prsResults_AUC_sd_high) = rownames_trait
rownames(prsResults_AUC_sd_low) = rownames_trait
colnames(prsResults_r2_low) = traits
colnames(prsResults_r2_high) = traits
...
createPlot(prsResults_r2, prsResults_r2_low, prsResults_r2_high, "r^2", filn="r2", T, xlimit = c(0,65), cexV = 1.2)
...
outputLoc="C:/Users/M Kel/GoogleDrive_Cam/0Publications/shaPRS/Figs/revisions_ajhg/AFR_res_gap"
baseloc="C:/Users/M Kel/GoogleDrive_Cam/0Publications/shaPRS/Figs/revisions_ajhg/gap"


# CROSS ANCESTRY BARPLOTS
imagesPerRow= 1 # how many images per row
outName="crossAncestryBarplot_publish_revAJHG_gap.png"
...
image_write(img, paste0("C:/Users/M Kel/GoogleDrive_Cam/0Publications/shaPRS/Figs/revisions_ajhg/",outName))


#####################
# simulation heatmaps

baseloc="C:/Users/M Kel/GoogleDrive_Cam/0Publications/shaPRS/Figs/revisions_ajhg/new/"
imagesPerRow= 2 # how many images per row

# r2diff
outName="sim_r2diff.png"


image2 <- image_read(paste0(baseloc,"1000_h2_0.5sims_eval_ukbb_ajhg_subset.png"))
image3 <- image_read(paste0(baseloc,"1000_h2_0.5sims_eval_ukbb_ajhg_subset_extra.png"))
...
image2 <- image_read(paste0(baseloc,"1000_h2_0.5sims_eval_ukbb_ajhg_lrt_subset.png"))
image3 <- image_read(paste0(baseloc,"1000_h2_0.5sims_eval_ukbb_ajhg_lrt_subset_extra.png"))
