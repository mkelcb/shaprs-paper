...
##' Compute prscsx in test after model training 
##'
...
compute_prscsx <- function(id_f, model_files, trait, out){
    ...
    ## Fit model to train set (logistic regression)

    form <- as.formula(paste(trait, "~", paste(score_cols, collapse = " + ")))
    ...
    write.table(test[, (score_cols) := NULL], out, row.names=FALSE)
...
compute_prscsx(snakemake@input[['test_ids']], snakemake@input[['scores']], snakemake@wildcards[['trait']], snakemake@output[['out']] )
